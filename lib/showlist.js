//This module gets the data from the Jambase API and saves it to itself. 
require('datejs');
...
function showList(zipCode, cacheName) {
  this.theShowList = [];
  ...
  if (config.redisURL) {
    ...
    client.get(this.cache, (err, reply) => {
...
showList.prototype.update = function update(callback) {
  //The Function
  var getData = (page) => {
    ...
    http
      .get(api, (res) => {
        ...
        res.on('end', () => {
          ...
          page = page + 1;
          if (page <= 6) {
            setTimeout(function(){getData(page)}, 500);
          } else {
            if (config.redisURL) {
